...
class LatentDiffusion(DDPM):
    ...
    def get_input(
        ...
        random = torch.rand(sum(cond_counts), device=x.device)
        ...
        with torch.enable_grad():
            ...
            null_prompt = torch.cat(
                [self.get_learned_conditioning([""]).detach()] * sum(cond_counts)
            )
            # print(xc.shape)
            # print(null_prompt.shape, clip_emb.shape)
        ...
        view_delimiters = (torch.cumsum(cond_counts, 0) - cond_counts[0]).tolist()
        first_stage_encoded = (
            self.encode_first_stage((xc.to(self.device))).mode().detach()
        )
        relative_views = first_stage_encoded[view_delimiters]
        relative_views_stacked = torch.repeat_interleave(
            relative_views, cond_counts, dim=0
        )
        full_relative_conditioning = torch.cat(
            [first_stage_encoded, relative_views_stacked], dim=1
        )
        # print(full_relative_conditioning.shape)
        cond["c_concat"] = [input_mask * full_relative_conditioning]
        # print(cond["c_concat"][0].shape)
        out = [z, cond, cond_counts]
    ...
    def shared_step(self, batch, **kwargs):
        x, c, cond_counts = self.get_input(batch, self.first_stage_key)
        # print("COND COUNTS 1", cond_counts)
        loss = self(x, c, cond_counts)
        return loss

    def forward(self, x, c, cond_counts, *args, **kwargs):
        t = torch.randint(
            0, self.num_timesteps, (x.shape[0],), device=self.device
        ).long()
        ...
        return self.p_losses(x, c, cond_counts, t, *args, **kwargs)
    ...
    def apply_model(self, x_noisy, t, cond, cond_counts, return_ids=False):
    ...
    def p_losses(self, x_start, cond, cond_counts, t, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))

        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        x_noisy = torch.repeat_interleave(x_noisy, cond_counts, dim=0)
        noise = torch.repeat_interleave(noise, cond_counts, dim=0)
        t_model = torch.repeat_interleave(t, cond_counts, dim=0)

        model_output = self.apply_model(x_noisy, t_model, cond, cond_counts)

        view_delimiters = torch.cumsum(cond_counts, 0).tolist()
        view_delimiters.insert(0, 0)
        noise_weight_delim = model_output.shape[1] // 2
        noise_all, logits = (
            model_output[:, :noise_weight_delim, ...],
            model_output[:, noise_weight_delim:, ...],
        )
        ...
        weights_softmax = torch.nn.functional.softmax(logits_padded, dim=1)
        ...
        model_output_aggregated = noise_weighted.sum(dim=1)
        # print("MODEL OUT AGG", model_output_aggregated.shape)

        loss_dict = {}
        prefix = "train" if self.training else "val"

        if self.parameterization == "x0":
            target = x_start[view_delimiters[:-1]]
        elif self.parameterization == "eps":
            target = noise[view_delimiters[:-1]]
        else:
            raise NotImplementedError()

        loss_simple = self.get_loss(model_output_aggregated, target, mean=False).mean(
            [1, 2, 3]
        )
        ...
        loss_vlb = self.get_loss(model_output_aggregated, target, mean=False).mean(
            dim=(1, 2, 3)
        )
    ...
    def p_mean_variance(
        ...
        noise_weight_delim = model_out.shape[1] // 2
        noise_all, logits = (
            model_out[:, :noise_weight_delim, ...],
            model_out[:, noise_weight_delim:, ...],
        )
        logits_padded = torch.nn.utils.rnn.pad_sequence(
            [
        ...
        model_out_aggregated = noise_weighted.sum(dim=1)
        ...
        if self.parameterization == "eps":
            x_recon = self.predict_start_from_noise(x, t=t, noise=model_out_aggregated)
        elif self.parameterization == "x0":
            x_recon = model_out_aggregated
        else:
            raise NotImplementedError()
    ...
    @torch.no_grad()
    def sample_log(self, cond, cond_counts, batch_size, ddim, ddim_steps, **kwargs):
        if ddim:
    ...
    def get_unconditional_conditioning(
        ...
        cond["c_crossattn"] = [c]
        cond["c_concat"] = [
            torch.zeros([batch_size, 8, image_size // 8, image_size // 8]).to(
                self.device
            )
        ]
        return cond
    ...
    def log_images(
        ...
        **kwargs,
        ...
        log = dict()
        z, c, cond_counts, x, xrec, xc = self.get_input(
            batch,
        ...
        if unconditional_guidance_scale > 1.0:
            # print("getting unconditional conditioning", x.shape, N)
            uc = self.get_unconditional_conditioning(
                sum(cond_counts), unconditional_guidance_label, image_size=x.shape[-1]
            )
