...
if __name__ == "__main__":
    ...
    try:
        ...
        if not opt.finetune_from == "":
            ...
            # Check if we need to port input weight channels
            ...
            if in_shape != in_filters_load.shape:
                ...
                for input_key in input_keys:
                    ...
                    if input_weight.size() != old_state[input_key].size():
                        ...
                        input_weight.zero_()
                        print(input_weight.shape, old_state[input_key].shape)
                        input_weight[:, : old_state[input_key].shape[1], :, :].copy_(
                            old_state[input_key]
                        )
            ...
            # Check if we need to port output weight channels
            out_weights = old_state["model.diffusion_model.out.2.weight"]
            out_weights_current = new_state["model.diffusion_model.out.2.weight"]
            out_shape = out_weights_current.shape
            if out_shape != out_weights.shape:
                input_keys = [
                ...
                for input_key in input_keys:
                    ...
                    if input_weight.size() != old_state[input_key].size():
                        ...
                        input_weight.zero_()
                        print(input_weight.shape, old_state[input_key].shape)
                        input_weight[: old_state[input_key].shape[0], ...].copy_(
                            old_state[input_key]
                        )
                        old_state[input_key] = torch.nn.parameter.Parameter(
                            input_weight
